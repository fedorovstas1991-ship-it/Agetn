...
const DUCKDUCKGO_SEARCH_ENDPOINT = "https://r.jina.ai/http://html.duckduckgo.com/html/";
...
function parseMarkdownLinks(block: string): Array<{ text: string; url: string }> {
  const results: Array<{ text: string; url: string }> = [];
  const linkRegex = /\[([^\]\n]+)\]\((https?:\/\/[^\s)]+)\)/g;
  let match: RegExpExecArray | null = null;
  while ((match = linkRegex.exec(block)) !== null) {
    const text = stripMarkdownFormatting(match[1] ?? "");
...
function parseDuckDuckGoMarkdown(markdown: string, maxCount: number) {
  const headingRegex = /\[([^\]\n]+)\]\((https?:\/\/[^\s)]+)\)\r?\n[-]{3,}/g;
  ...
  for (let idx = 0; idx < matches.length && results.length < maxCount; idx += 1) {
    const match = matches[idx];
    const start = match.index ?? 0;
    const end = idx + 1 < matches.length ? (matches[idx + 1].index ?? markdown.length) : markdown.length;
    const block = markdown.slice(start, end);
    const links = parseMarkdownLinks(block);
    if (links.length === 0) {
      continue;
    }
    const primary = links[0];
    const url = decodeDuckDuckGoRedirect(primary.url);
    if (!/^https?:\/\//i.test(url)) {
      continue;
    }
    const dedupeKey = `${primary.text}|${url}`;
      ...
      links
        ...
        .find(
          (text) =>
            ...
            !looksLikeDomainOrPath(text) &&
            text.length >= 16,
        ) ?? "";

    results.push({
      title: primary.text ? wrapWebContent(primary.text, "web_search") : "",
      url,
...
async function runDuckDuckGoSearch(params: {
  ...
  const mapped = parseDuckDuckGoMarkdown(markdown, params.count);
  if (mapped.length === 0) {
    ...
    execute: async (_toolCallId, args) => {
      ...
      const providerForRun =
        provider === "brave" && !apiKey
          ? ("duckduckgo" as (typeof SEARCH_PROVIDERS)[number])
          : provider;

      if (!apiKey && providerForRun !== "duckduckgo") {
      ...
      if (rawFreshness && providerForRun !== "brave") {
        return jsonResult({
