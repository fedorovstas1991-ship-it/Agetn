...
URL Source: http://html.duckduckgo.com/html/?q=openclaw

Markdown Content:
[OpenClaw - GitHub](http://duckduckgo.com/l/?uddg=https%3A%2F%2Fgithub.com%2Fopenclaw%2Fopenclaw&rut=abc)
---
[Open-source personal AI assistant.](http://duckduckgo.com/l/?uddg=https%3A%2F%2Fgithub.com%2Fopenclaw%2Fopenclaw&rut=abc)`),
      } as Response),
    );
    // @ts-expect-error mock fetch
    global.fetch = mockFetch;
    ...
    const result = await tool?.execute?.(1, { query: "openclaw", count: 3 });
    ...
    expect(details.results?.[0]?.url).toBe("https://github.com/openclaw/openclaw");
    expect(mockFetch).toHaveBeenCalledWith(
      expect.stringContaining("https://r.jina.ai/http://html.duckduckgo.com/html/?q=openclaw"),
      expect.any(Object),
    );
  });
});
