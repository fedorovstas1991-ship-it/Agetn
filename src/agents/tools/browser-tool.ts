...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    description: [
      ...
      'For action="open" without an explicit profile, default to profile="openclaw" so links can open without Chrome extension attach.',
    ...
    execute: async (_toolCallId, args) => {
      ...
      switch (action) {
        ...
        case "open": {
          ...
          const openProfile = profile ?? "openclaw";
          if (proxyRequest) {
            const result = await proxyRequest({
              method: "POST",
              path: "/tabs/open",
          ...
          return jsonResult(await browserOpenTab(baseUrl, targetUrl, { profile: openProfile }));
        }
        case "focus": {
