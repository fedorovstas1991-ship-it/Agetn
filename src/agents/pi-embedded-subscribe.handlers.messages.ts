...
export function handleMessageEnd(
  ...
  const errorMessageRaw =
    typeof (assistantMessage as { errorMessage?: unknown }).errorMessage === "string"
      ? (assistantMessage as { errorMessage?: string }).errorMessage
      : "";
  ctx.state.lastAssistantStopReason = stopReasonRaw.trim().toLowerCase() || undefined;
  ctx.state.lastAssistantErrorMessage = errorMessageRaw.trim() || undefined;
