...
- Скрипт `yagent-onboard-ui.command` (обновлён 2026-02-26):
  ...
  - Runtime-логи перенесены из репозитория в профиль: `~/.yagent-yagent/logs` (вместо `./logs/yagent`).
  - Сборка Control UI перенесена из `./dist/control-ui` в `~/.yagent-yagent/control-ui`.
  - Порядок операций исправлен: сначала `fresh reset`, потом сборка Control UI (иначе reset удалял только что собранные UI-ассеты).
  - Launcher принудительно фиксирует `gateway.controlUi.root` в `openclaw.json` на внешний путь Control UI, чтобы gateway не зависел от `dist/control-ui` в рабочей копии.
  - Лишние системные следы OpenClaw очищены: `~/.openclaw*`, `~/Documents/OpenClaw`, `~/tmp/openclaw-501`, старые `~/Library/LaunchAgents/ai.openclaw*.plist`.
...
**Симптом: «NDA работает, но сильно грузит систему» (2026-02-26)**
- **Файлы:** `src/agents/pi-embedded-subscribe.handlers.ts`, `src/agents/pi-embedded-subscribe.handlers.messages.ts`, `src/agents/pi-embedded-runner/run/attempt.ts`, `src/gateway/server-methods/chat.ts`.
- **Причина:** в NDA используется локальная `ollama/qwen2.5:7b-instruct` (около 4.8 GB в рантайме), плюс в коде остались временные `[DEBUG ...]` логи, которые создавали лишний шум в stderr/log.
- **Фикс:** удалены временные debug-логи из обработчиков embedded-потока и `chat.send` в gateway.
- **Операционный совет:** чтобы быстро снять нагрузку, выгружать модель когда idle: `ollama stop qwen2.5:7b-instruct` (следующий NDA-запрос поднимет модель снова).

**Баг: ложный `Gateway failed to start` после выноса UI в профиль**
- **Файл:** `yagent-onboard-ui.command`
- **Причина:** сборка Control UI выполнялась до `fresh reset`; затем `rm -rf ~/.yagent-yagent` удалял `~/.yagent-yagent/control-ui`, и проверка ошибочно падала, хотя gateway уже слушал порт.
- **Фикс:** блок сборки UI перенесён ниже, после reset и после фиксации `gateway.controlUi.root`.
- **Проверка:** повторный запуск `./yagent-onboard-ui.command` завершился успешно; скрипт открыл onboarding URL и прошёл как новый пользователь.

**Баг: агент утверждает «one-search MCP не существует» и ищет альтернативы**
- **Файл:** `docs/reference/templates/TOOLS.md`
- **Причина:** шаблон `TOOLS.md` был пустым placeholder (`# Tools\n\nDefine tools here.`). Агент не получал никаких знаний о подключённых MCP-серверах в bootstrap — при попытке использовать веб-поиск он не находил `one-search` в «официальном реестре» и пытался установить `@modelcontextprotocol/server-brave` или другие альтернативы.
- **Фикс:** в шаблон `docs/reference/templates/TOOLS.md` прописано явное знание о MCP-сервере `one-search`: что он существует (`one-search-mcp@1.1.2` на npm), работает через DuckDuckGo без API-ключа, уже подключён в конфиге, и не нужно искать альтернативы.
- **Механизм:** шаблон `TOOLS.md` копируется в workspace каждого агента при создании (`ensureAgentWorkspace` → `writeFileIfMissing`). Файл входит в `SUBAGENT_BOOTSTRAP_ALLOWLIST` — знание доступно и основному агенту, и субагентам.
- **Контекст:** в проекте две независимые системы веб-поиска:
  1. **MCP `one-search`** (DuckDuckGo, без ключа) — дефолт для zero-config: `applyDefaultMcpServers()` в `src/config/defaults.ts` инжектирует `npx -y one-search-mcp` с `SEARCH_PROVIDER=duckduckgo` если `cfg.mcpServers` пустой.
  2. **Нативный `web_search` tool** (`src/agents/tools/web-search.ts`) — Brave/Perplexity/Grok, требует API-ключ. Для NDA-агента заблокирован через `tools.deny += ["group:web"]`.

**Баг: `web_search` падал с `missing_brave_api_key` и уходил в долгие попытки**
- **Файлы:** `src/agents/tools/web-search.ts`, `src/config/types.tools.ts`, `src/config/zod-schema.agent-runtime.ts`, `src/config/config.web-search-provider.test.ts`.
- **Причина:** в runtime инструменты MCP `one-search` не попадали в список tool calls, а встроенный `web_search` по умолчанию требовал Brave API key.
- **Фикс:** добавлен автоматический fallback на DuckDuckGo в `web_search` (через `r.jina.ai` + `html.duckduckgo.com`) при отсутствии Brave key; добавлена поддержка `tools.web.search.provider = "duckduckgo"` в типах/схеме.
- **Проверка:** целевые и смежные тесты `web-search`/config проходят (`177 passed`).

**Баг: `browser open` требовал Chrome extension attach даже для простого открытия ссылки**
- **Файлы:** `src/agents/tools/browser-tool.ts`, `src/agents/tools/browser-tool.test.ts`.
- **Причина:** `action="open"` без `profile` использовал дефолтный профиль runtime (`chrome`), который в extension-режиме требует ручной attach вкладки.
- **Фикс:** для `action="open"` без явного `profile` теперь используется `profile="openclaw"`; явный `profile="chrome"` по-прежнему работает для takeover-сценариев.
- **Проверка:** добавлен и проходит unit-тест `uses openclaw profile by default for open action`.

**Баг: секция чата «дёргалась» при наведении курсора после онбординга**
- **Файлы:** `ui/src/ui/views/chat.ts`, `ui/src/styles/chat/layout.css`, `ui/src/styles/components.css`, `ui/src/ui/views/chat.test.ts`.
- **Причина:** чат рендерится как `section.card.chat`, а глобальный стиль `.card:hover` в `components.css` применял `transform: translateY(-2px)`, из-за чего весь чат-контейнер слегка смещался под курсором.
- **Фикс:** chat-контейнер помечен классом `chat--static`, добавлен override `.card.chat.chat--static:hover { transform: none; }`, чтобы убрать hover-сдвиг именно у чата.
- **Проверка:** добавлен тест `renders chat card as static to prevent hover jitter` в `ui/src/ui/views/chat.test.ts`.

**Баг: после онбординга авто-сообщение оставалось в поле ввода и не отправлялось**
- **Файлы:** `ui/src/ui/app.ts`, `ui/src/ui/onboarding-autosend.browser.test.ts`.
- **Причина:** в `_handleOnboardingComplete()` был один `setTimeout(...handleSendChat())`; если в этот момент gateway был в состоянии reconnect/рестарта, `handleSendChat()` выходил по `!connected`, и сообщение оставалось в draft.
- **Фикс:** добавлена очередь авто-приветствия с retry-попытками и лимитом (`ONBOARDING_WELCOME_MAX_ATTEMPTS`), повторный триггер на `connected=true`, очистка таймера при `disconnectedCallback`.
- **Проверка:** `pnpm --dir ui exec vitest run src/ui/onboarding-autosend.browser.test.ts --config vitest.config.ts` (`2 passed`).

**Проверка онбординга после фиксов (2026-02-26, повторный прогон)**
- Запуск: `./yagent-onboard-ui.command`.
...
- Репозиторий проекта: `https://github.com/fedorovstas1991-ship-it/Agetn`.
