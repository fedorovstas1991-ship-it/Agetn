...
STATE_DIR="${OPENCLAW_YAGENT_STATE_DIR:-$HOME/.yagent-${PROFILE}}"
LOG_DIR="${OPENCLAW_YAGENT_LOG_DIR:-$STATE_DIR/logs}"
CONTROL_UI_OUT_DIR="${OPENCLAW_CONTROL_UI_OUT_DIR:-$STATE_DIR/control-ui}"
...
echo "Control UI dir: $CONTROL_UI_OUT_DIR"
  ...
  "agents": {
    ...
    "list": [
      ...
      },
      {
        ...
        "tools": {
          "profile": "minimal",
          "deny": ["group:web", "tts", "session_status"]
        }
      }
    ]
  },
...
# Launcher policy: keep Control UI artifacts outside repository paths.
ensure_control_ui_root_in_config() {
  local cfg="$1"
  local ui_root="$2"
  if [[ ! -f "$cfg" ]]; then
    return
  fi
  "$NODE_BIN" - "$cfg" "$ui_root" <<'NODE'
const fs = require("fs");
const cfgPath = process.argv[2];
...
try {
  const raw = fs.readFileSync(cfgPath, "utf8");
  const parsed = JSON.parse(raw);
  const existingRoot = parsed?.gateway?.controlUi?.root;
  if (existingRoot === uiRoot) {
    process.exit(0);
  }
  ...
  if (
    ...
    typeof parsed.gateway.controlUi !== "object" ||
    Array.isArray(parsed.gateway.controlUi)
  ) {
    parsed.gateway.controlUi = {};
  }
  parsed.gateway.controlUi.root = uiRoot;
  fs.writeFileSync(cfgPath, `${JSON.stringify(parsed, null, 2)}\n`, "utf8");
} catch (error) {
  const message = error && typeof error === "object" && "message" in error ? error.message : String(error);
  console.error(`Failed to enforce gateway.controlUi.root in ${cfgPath}: ${message}`);
  process.exit(1);
}
NODE
}

ensure_control_ui_root_in_config "$OPENCLAW_CONFIG_PATH" "$CONTROL_UI_OUT_DIR"

# Always rebuild Control UI so Telegram flow uses current frontend code.
# Build runs after hard reset so artifacts are not deleted by cleanup.
# Set OPENCLAW_SKIP_UI_BUILD=1 only for fast local debugging.
if [[ "${OPENCLAW_SKIP_UI_BUILD:-0}" != "1" ]]; then
  echo "Building Control UI..."
  echo "Control UI output dir: $CONTROL_UI_OUT_DIR"
  mkdir -p "$(dirname "$CONTROL_UI_OUT_DIR")"
  rm -rf "$CONTROL_UI_OUT_DIR"
  OPENCLAW_CONTROL_UI_OUT_DIR="$CONTROL_UI_OUT_DIR" node scripts/ui.js build
fi
