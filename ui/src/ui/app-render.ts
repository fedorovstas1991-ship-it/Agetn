...
function isNdaSession(state: AppViewState): boolean {
  const parsed = parseAgentSessionKey(state.sessionKey);
  return (parsed?.agentId ?? "").trim().toLowerCase() === "nda";
}
...
function shouldShowFirstGreetingCta(state: AppViewState): boolean {
  ...
  if (!state.connected || state.chatStream !== null) {
    return false;
  }
...
function resolveCronWizardStep(wizard: AppViewState["cronWizard"]): 1 | 2 | 3 | 4 {
  const textReady = wizard.text.trim().length > 0;
  if (wizard.schedulePreset === "in") {
    if (!wizard.inAmount.trim()) {
      return 2;
    }
    return textReady ? 4 : 3;
  }
  if (wizard.schedulePreset === "daily") {
    if (!wizard.dailyTime.trim()) {
      return 2;
    }
    return textReady ? 4 : 3;
  }
  if ((wizard.weekDays?.length ?? 0) === 0 || !wizard.weekTime.trim()) {
    return 2;
  }
  return textReady ? 4 : 3;
}
...
export function renderApp(state: AppViewState) {
  ...
  const chatDisabledReason = state.connected ? null : "Нет соединения с gateway.";
    ...
    <div class="shell ${isChat ? "shell--chat" : ""} ${chatFocus ? "shell--chat-focus" : ""} ${state.settings.navCollapsed ? "shell--nav-collapsed" : ""} ${state.onboarding ? "shell--onboarding" : ""}">
      ...
      state.applySettings({
        ...
        navCollapsed: !state.settings.navCollapsed,
            ...
            title="${state.settings.navCollapsed ? RU_UI.expandSidebar : RU_UI.collapseSidebar}"
            aria-label="${state.settings.navCollapsed ? RU_UI.expandSidebar : RU_UI.collapseSidebar}"
          >
          ...
          <div class="brand">
            ...
            <div class="brand-text">
              <div class="brand-title">YANDEX AGENT</div>
            </div>
          </div>
        </div>
      ...
      <aside class="nav ${state.settings.navCollapsed ? "nav--collapsed" : ""}">
        ${TAB_GROUPS.filter((group) => group.label !== RU_UI.groupAdvanced || state.showAdvancedNav).map(
        (group) => {
        ...
        <div class="nav-group nav-group--links">
          <div class="nav-label nav-label--static">
            <span class="nav-label__text">${RU_UI.resources}</span>
          </div>
          <div class="nav-group__items">
            <a
      ...
      ? renderChannels({
        ...
        telegramConnectFlow: state.telegramConnectFlow,
        telegramConnectStatus: state.telegramConnectStatus,
        telegramConnectDetails: state.telegramConnectDetails,
        ...
        onTelegramConnect: () => state.handleTelegramConnect(),
      ...
      ? renderCron({
        ...
        onWizardChange: (patch) => {
          const next = {
            ...state.cronWizard,
            ...patch,
            error: null,
          };
          next.step = resolveCronWizardStep(next);
          state.cronWizard = next;
        },
        onWizardModeChange: (mode) =>
          (state.cronWizard = {
            ...state.cronWizard,
            mode,
            error: null,
        ...
        onAddWizard: () => addCronJobFromWizard(state),
        onDraftInChatTemplate: () => {
          const template = buildCronPromptTemplate(state.cronWizard);
          state.setTab("chat");
          state.chatMessage = template;
      ...
      ? renderChat({
        ...
        onSessionKeyChange: (next) => {
          state.sessionKey = next;
          state.productChatMode = next.startsWith("agent:nda:") ? "nda" : "regular";
          state.chatMessage = "";
        ...
        chatMode: isNdaSession(state) ? "nda" : state.productChatMode,
