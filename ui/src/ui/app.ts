...
const ONBOARDING_WELCOME_MESSAGE = "Привет! Что ты умеешь?";
...
export class OpenClawApp extends LitElement {
  ...
  private clearOnboardingWelcomeRetryTimer() {
    if (this.onboardingWelcomeRetryTimer == null) {
      return;
    }
    window.clearTimeout(this.onboardingWelcomeRetryTimer);
  ...
  private queueOnboardingWelcomeMessage() {
  ...
  private async trySendOnboardingWelcome() {
    const pendingMessage = this.onboardingWelcomePendingMessage;
    if (!pendingMessage) {
      return;
    }
    ...
    if (!this.connected || this.chatSending || Boolean(this.chatRunId)) {
      this.scheduleOnboardingWelcomeRetry(ONBOARDING_WELCOME_RETRY_DELAY_MS);
      return;
    }
    ...
    this.onboardingWelcomeSendAttempts += 1;
    this.chatMessage = pendingMessage;
  ...
  private _handleOnboardingComplete() {
    ...
    this.queueOnboardingWelcomeMessage();
  }
}
